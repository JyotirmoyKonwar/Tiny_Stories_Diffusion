    ...
    while len(all_tokens) - prompt_len < max_new_tokens:
        ...
        while masked.any():
            ...
            if mode == "Show Generation Process":
                # Combine the original prompt and the newly generated masked blocks progressively
                current_block = x[0, prompt_len:prompt_len + block_len].tolist()
...
def gradio_fn(prompt, display_mode):
    for text in generate_diffusion(prompt, max_new_tokens=100, mode=display_mode):
        yield text
...
with gr.Blocks(theme=gr.themes.Monochrome()) as demo:
    ...
    generate_btn.click(fn=gradio_fn, inputs=[prompt_in, mode], outputs=output)
